  ...
  function Quiz({ route }) {
    ...
    const saveUserToRanking = async () => {
      try {
        const rankingData = await FileSystem.readAsStringAsync(FileSystem.documentDirectory + 'ranking.json');
        let ranking = [];
        if (rankingData) {
          ranking = JSON.parse(rankingData);
        }
        ranking.push( {
          nome,
          score,
        ...
        await FileSystem.writeAsStringAsync(
          FileSystem.documentDirectory + 'ranking.json'),
          JSON.stringify(ranking);

        console.log('Adicionado ao ranking:', {nome, score});
      } catch (error) {
